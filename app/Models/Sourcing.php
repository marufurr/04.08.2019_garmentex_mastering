...
use App\Observer\SourcingObserver;
...
class Sourcing extends BaseModel implements Feedable
{
	...
	*/
	...
	{
		$sourcingsPerPage = (int)config('settings.listing.items_per_page', 50);
		...
		if (
			...
			$sourcings = Sourcing::where('country_code', $countryCode)
				->unarchived()
				->take($sourcingsPerPage)
				->orderByDesc('id')
				->get();
		} else {
			$sourcings = Sourcing::unarchived()->take($sourcingsPerPage)->orderByDesc('id')->get();
		}
	...
	public function getTitleHtml()
	{
		...
		$sourcing = self::find($this->id);
		$out .= getSourcingUrl($sourcing);
		$out .= '<br>';
		$out .= '<small>';
	...
	public function getPictureHtml()
	{
		...
		if ($this->pictures->count() > 0) {
			foreach ($this->pictures as $picture) {
				$url = localUrl($picture->sourcing->country_code, $this->uri);
	...
	*/
	...
	{
		return $this->hasMany(Message::class, 'sourcing_id');
	}
	
	public function latestPayment()
	{
		return $this->hasOne(Payment::class, 'sourcing_id')->orderBy('id', 'DESC');
	}
	
	public function payments()
	{
		return $this->hasMany(Payment::class, 'sourcing_id');
	}
	
	public function pictures()
	{
		return $this->hasMany(Picture::class, 'sourcing_id')->orderBy('position')->orderBy('id', 'DESC');
	}
	
	public function savedByUsers()
	{
		return $this->hasMany(SavedSourcing::class, 'sourcing_id');
	}
	...
	*/
	public function scopeVerified($builder)
	{
		...
		if (config('settings.single.sourcings_review_activation')) {
