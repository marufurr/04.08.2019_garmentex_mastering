...
use App\Notifications\EmaiEntitylVerification;
	...
	public function sendVerificationEmail($entity, $displayFlashMessage = true)
	{
		...
		try {
			if (request()->filled('locale')) {
				...
				if (!empty($locale)) {
				    $entity->notify((new EmaiEntitylVerification($entity, $entityRef))->locale($locale));
				} else {
				    $entity->notify(new EmaiEntitylVerification($entity, $entityRef));
				}
			} else {
			    $entity->notify(new EmaiEntitylVerification($entity, $entityRef));
			}
			...
			if ($displayFlashMessage) {
				$message = "An activation code has been sent to you to verify your email address.";
